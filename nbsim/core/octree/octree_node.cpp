...
OctreeNode::OctreeNode(double width, const Vec3 &center)
    : type{OctreeNodeType::EXTERNAL}, box{BoundingBox(center, width)},
      localObj{nullptr} {
...
OctreeNode::OctreeNode(const OctreeNode &other)
    : type{other.type}, box{BoundingBox(other.box.center, other.box.width)},
      localObj{nullptr} {
  for (size_t i = 0; i < 8; i++) {
    children[i] = nullptr;
  }
...
void OctreeNode::insertOctant(Object *obj) {
  ...
  if (!(children[index])) {
    ...
    case Octant::FIRST: {
      newCenter =
          box.center + Vec3{box.width / 4.0, box.width / 4.0, box.width / 4.0};
      break;
    }
    case Octant::SECOND: {
      newCenter = box.center +
                  Vec3{box.width / 4.0, box.width / 4.0, -1 * box.width / 4.0};
      break;
    }
    case Octant::THIRD: {
      newCenter = box.center +
                  Vec3{box.width / 4.0, -1 * box.width / 4.0, box.width / 4.0};
      break;
    }
    case Octant::FOURTH: {
      newCenter = box.center + Vec3{box.width / 4.0, -1 * box.width / 4.0,
                                    -1 * box.width / 4.0};
      break;
    }
    case Octant::FIFTH: {
      newCenter = box.center +
                  Vec3{-1 * box.width / 4.0, box.width / 4.0, box.width / 4.0};
      break;
    }
    case Octant::SIXTH: {
      newCenter = box.center + Vec3{-1 * box.width / 4.0, box.width / 4.0,
                                    -1 * box.width / 4.0};
      break;
    }
    case Octant::SEVENTH: {
      newCenter = box.center + Vec3{-1 * box.width / 4.0, -1 * box.width / 4.0,
                                    box.width / 4.0};
      break;
    }
    default: {
...
Octant OctreeNode::getOctant(Object *obj) {
  if (obj->position.x < box.center.x && obj->position.y < box.center.y &&
    ...
    // is in octant 8
    return Octant::EIGHT;
  } else if (obj->position.x < box.center.x && obj->position.y < box.center.y &&
             obj->position.z >= box.center.z) {
    // is in octant 7
    ...
    return Octant::FIFTH;
             ...
             obj->position.y < box.center.y && obj->position.z < box.center.z) {
    // is in octant 4
