...
Octree::Octree()
    : allocSize{8}, size{0}, width{1000}, bodies{new Body[allocSize]},
      root{nullptr} {}

Octree::Octree(double simWidth)
    : allocSize{8}, size{0}, width{simWidth}, bodies{new Body[allocSize]},
      root{nullptr} {}

Octree::Octree(vector<Body> &inputBodies)
    : allocSize{inputBodies.size()}, size{0}, bodies{new Body[allocSize]},
      root{nullptr} {
...
Octree::Octree(const Octree &other)
    : allocSize{other.allocSize}, size{other.size}, width{other.width},
      bodies{new Body[allocSize]}, root{nullptr} {
...
Octree::Octree(Octree &&other)
    : allocSize{other.allocSize}, size{other.size}, width{other.width},
      bodies{other.bodies}, root{other.root} {
  other.root = nullptr;
...
double Octree::calculateWidth() const {
  ...
  for (size_t i = 0; i < size; i++) {
    double max_obj_coord =
        max(abs(bodies[i].position.x),
            max(abs(bodies[i].position.y), abs(bodies[i].position.z)));
