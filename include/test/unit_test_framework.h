...
auto print_helper(std::ostream& os, const std::pair<First, Second>& t, int, int)
...
void assert_sequence_equal(First&& first, Second&& second, int line_number,
    ...
    for (; it1 != end1 and it2 != end2; ++it1, ++it2, ++position) {
        if (not safe_equals<decltype(*it1), decltype(*it2)>::equals(*it1,
                                                                    *it2)) {
            equal = false;
            break;
        }
...
#else
std::string demangle(const char* typeinfo_name) { return typeinfo_name; }
